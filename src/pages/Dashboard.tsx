...
const complaintSchema = z.object({
  message: z.string().trim().min(10, {
    ...
    message: "Complaint must be less than 1000 characters"
  })
});
const Dashboard = () => {
  const {
  ...
  useEffect(() => {
    if (user) {
      // Fetch profile
      supabase.from('profiles').select('*').eq('id', user.id).maybeSingle().then(async ({
        data
      }) => {
        setProfile(data);
        ...
        const {
          data: passInfo
        } = await supabase.from('passes').select('*').eq('user_id', user.id).maybeSingle();
        ...
        if (data?.bus_number) {
          ...
          const possibleFileNames = [`bus-${data.bus_number}.png`, `${data.bus_number}.png`, 'route.png'];
          for (const fileName of possibleFileNames) {
            const {
              data: publicUrl
            } = supabase.storage.from('route').getPublicUrl(fileName);
  ...
  return <div className="min-h-screen bg-background bg-mesh">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-8 space-y-4 sm:space-y-8">
        ...
        {/* Pass Expiry Alert */}
        {passExpiryStatus?.isExpired && <Alert variant="destructive" className="border-destructive animate-slide-up shadow-lg">
        ...
        {showPassButton && <Card className="glass animate-slide-up shadow-glow border-primary/50">
            <CardContent className="py-6">
              <div className="flex items-center justify-between">
                ...
                <Button onClick={() => navigate('/expired-pass-letter')} className="ml-4 bg-gradient-primary hover:shadow-glow font-bold text-lg px-8 py-6 h-auto">
                  PASS
                </Button>
              </div>
            </CardContent>
        ...
        {passExpiryStatus?.isExpiringSoon && !passExpiryStatus?.isExpired && <Alert className="border-yellow-500 bg-yellow-500/10 animate-slide-up shadow-md">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-6">
          ...
          <Card className="glass hover:shadow-glow transition-all duration-300 cursor-pointer group animate-slide-up hover:scale-105 overflow-hidden relative" onClick={() => navigate('/bus-details')} style={{
          animationDelay: '0.2s'
        }}>
          ...
          <Card className="glass hover:shadow-glow transition-all duration-300 cursor-pointer group animate-slide-up hover:scale-105 overflow-hidden relative" onClick={() => navigate('/epass')} style={{
          animationDelay: '0.3s'
        }}>
            <div className="absolute inset-0 bg-gradient-accent opacity-0 group-hover:opacity-10 transition-opacity" />
            <CardHeader>
              <CardTitle className="flex items-center gap-3 font-display">
                <div className="p-2 rounded-lg transition-colors bg-[#05827c]">
        ...
        {profile?.bus_number && <Card className="glass animate-slide-up shadow-lg" style={{
        animationDelay: '0.4s'
      }}>
            <CardHeader>
            ...
            <CardContent>
              {routeImageUrl ? <div className="rounded-xl overflow-hidden shadow-md hover:shadow-glow transition-shadow">
                  <img src={routeImageUrl} alt={`Route map for bus ${profile.bus_number}`} className="w-full h-auto" />
                  <p className="text-center text-sm text-muted-foreground mt-4 font-medium">
                    Bus Number: {profile.bus_number}
                  </p>
                </div> : <div className="bg-gradient-primary/5 rounded-xl p-12 text-center">
        ...
        <Card className="glass animate-slide-up shadow-lg" style={{
        animationDelay: '0.5s'
      }}>
          <CardHeader>
          ...
          <CardContent>
            <form onSubmit={handleComplaintSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea id="complaint" placeholder="Please describe the issue you're facing (minimum 10 characters)..." value={complaint} onChange={e => {
                setComplaint(e.target.value);
                setValidationError(null);
              }} rows={4} maxLength={1000} className={`resize-none transition-all ${validationError ? 'border-destructive' : 'focus:border-primary'}`} />
              ...
              <Button type="submit" disabled={submitting || !complaint.trim()} className="bg-gradient-primary hover:shadow-glow transition-all font-medium">
                {submitting ? 'Submitting...' : 'Submit Complaint'}
              </Button>
            </form>

            {/* Previous Complaints */}
            {userComplaints.length > 0 && <div className="mt-6 pt-6 border-t border-border/50">
                <h3 className="font-semibold mb-3 text-foreground">Your Previous Complaints</h3>
                <div className="space-y-3">
                  {userComplaints.map(complaint => <Card key={complaint.id} className="glass border-border/50">
                      <CardContent className="pt-4">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <Button variant="ghost" size="sm" onClick={() => handleDeleteComplaint(complaint.id)} className="hover:bg-destructive/10 hover:text-destructive">
