...
const Index = () => {
  ...
  return <div className="relative flex min-h-screen items-center justify-center bg-background bg-mesh overflow-hidden" onClick={createRipple} onTouchStart={createRipple}>
      {/* Animated gradient overlay */}
      <div className="absolute inset-0 bg-gradient-primary opacity-5 animate-pulse" style={{
      animationDuration: '4s'
    }} />
      
      {/* Smoke trail effects */}
      {particles.map(particle => <div key={`particle-${particle.id}`} className="absolute pointer-events-none animate-smoke" style={{
      left: particle.x,
      top: particle.y,
      transform: 'translate(-50%, -50%)'
    }}>
          <div className="w-8 h-8 rounded-full bg-gradient-radial from-primary/30 via-primary/15 to-transparent" style={{
        filter: 'blur(8px)'
      }} />
        </div>)}
      
      {/* Touch ripple effects */}
      {ripples.map(ripple => <div key={`ripple-${ripple.id}`} className="absolute pointer-events-none" style={{
      left: ripple.x,
      top: ripple.y,
      transform: 'translate(-50%, -50%)'
    }}>
      ...
      <div className="absolute top-1 left-1 md:top-2 md:left-2 z-10 animate-fade-in">
        <div className="relative">
          <div className="absolute inset-0 bg-primary/40 blur-3xl rounded-full opacity-80 animate-pulse" style={{
          animationDuration: '3s'
        }} />
          <div className="absolute inset-0 bg-accent/30 blur-2xl rounded-full" />
          <img src={vishnuLogo} alt="VISHNU Universal Learning Logo" className="relative h-28 md:h-36 w-auto opacity-95 hover:opacity-100 transition-all hover:scale-105" style={{
      ...
      <div className="relative z-10 text-center space-y-6 md:space-y-8 px-4 max-w-4xl mx-auto pb-32 md:pb-24 animate-slide-up">
        ...
        <div className="flex flex-col sm:flex-row gap-3 md:gap-4 justify-center pt-2 md:pt-4">
          <Link to="/login">
            <Button size="lg" className="bg-primary hover:bg-primary/90 hover:shadow-glow transition-all w-full sm:w-auto text-base md:text-lg px-6 md:px-8 py-5 md:py-6">
              Login
            </Button>
          </Link>
          <Link to="/signup">
            <Button size="lg" variant="outline" className="border-primary/30 hover:bg-primary/10 hover:shadow-glow transition-all w-full sm:w-auto text-base md:text-lg px-6 md:px-8 py-5 md:py-6">
              Sign Up as Student
            </Button>
          </Link>
          <Link to="/signup-faculty">
            <Button size="lg" variant="outline" className="border-secondary/30 hover:bg-secondary/10 hover:shadow-glow transition-all w-full sm:w-auto text-base md:text-lg px-6 md:px-8 py-5 md:py-6">
              Sign Up as Faculty
            </Button>
          </Link>
        </div>
